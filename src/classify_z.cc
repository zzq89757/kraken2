...
// running options such as file path and params
struct Options
{
...
int main(int argc, char **argv)
{
  ...
  cerr << "---------- Kraken2 classify module modified by zzq ----------\n";
  ...
  for (int i = optind; i < argc; i++)
  {
    ...
    // init ClassificationStats and outputStream
    ClassificationStats stats = {0, 0, 0};
    OutputStreamData outputs = {false, false, nullptr, nullptr, nullptr, nullptr, &std::cout};
    // get prefix and concat full path
    // string prefix = removeString(getPrefix(getName(argv[i])));
    string prefix = getPrefix(getName(argv[i]));
    // string prefix = getName(argv[i]);
    opts.kraken_output_filename = out_dir + prefix + ".out";
    opts.report_filename = out_dir + prefix + ".report";
    // cerr<<opts.kraken_output_filename<<endl;
    cerr << prefix << " Classify Start ..." << endl;
    // cerr << opts.unclassified_output_filename << endl;
    if (opts.paired_end_processing)
    {
      if (!opts.unclassified_output_filename.empty())
      {
        outputs.unclassified_output1 = new ofstream(out_dir + prefix + "_rh1.fq");
        outputs.unclassified_output2 = new ofstream(out_dir + prefix + "_rh2.fq");
      }
...
// parse kraken command to classied params
void ParseCommandLine(int argc, char **argv, Options &opts)
{
  int opt;
  while ((opt = getopt(argc, argv, "h?H:t:o:T:p:R:C:U:O:Q:g:nmzqsPSMK")) != -1)
  {
    switch (opt)
    {
    case 'h':
